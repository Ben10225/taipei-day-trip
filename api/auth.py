...
bcrypt = Bcrypt()
router_page_auth = Blueprint("router_page_auth", __name__, template_folder="templates")


# sign_up api
@router_page_auth.route("/api/user", methods=["post"])
def sign_up():
...
# sign_in api
@router_page_auth.route("/api/user/auth", methods=["put"])
def sign_in():
  email = request.json['email']
  password = request.json['password']
...
# auth api
@router_page_auth.route("/api/user/auth")
def check_auth():
  try:
...
# sign_out api
@router_page_auth.route("/api/user/auth", methods=["delete"])
def sign_out():
...
@router_page_auth.route("/api/user/auth/cookie")
def get_ReserveData():
