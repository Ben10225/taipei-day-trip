...
def create_orders(order, bool):
  number = generate_order_code()
  create_p_value = [number, int(order["totalPrice"]), order["contact"]["name"], order["contact"]["email"], order["contact"]["phone"], bool]
  payment_id = Order.create_payment(create_p_value[0], create_p_value[1], create_p_value[2], create_p_value[3], create_p_value[4], bool)
